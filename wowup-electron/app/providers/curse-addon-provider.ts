/**
 * Curse Addon Provider for Main Process
 *
 * Ported from renderer process to main process for better performance.
 * All network operations now run in the main process instead of blocking the renderer.
 */
...
import { CircuitBreakerWrapper, NetworkService } from "../services/network-service";
import { TocService } from "../services/toc-service";

// Import utility functions directly
function strictFilter<T>(arr: (T | undefined)[]): T[] {
  return arr.filter((item): item is T => item !== undefined);
}

// TocNotFoundError class (ported from renderer)
...
export class CurseAddonProvider extends AddonProvider {
  ...
  public override async scan(
    ...
    try {
      ...
      log.error("failed to process fingerprint response");
      log.error(e);
      log.info(result);
      throw e;
    }
  ...
  public override async getChangelog(
    ...
    try {
      ...
      log.error("Failed to get changelog", e);
    }
  ...
  private hasSortableGameVersion(file: cfv2.CF2File, typeId: number): boolean {
    if (!file?.sortableGameVersions) {
      log.debug("sortableGameVersions missing", file);
    }
  ...
  private createAddon(
    ...
    if (!targetToc) {
      log.error("targetToc undefined", cfAddon.name, addonFolder.tocs);
  ...
  private rebuildLinkOut(description: string, href: string) {
    ...
    // Changed from window.decodeURIComponent to global.decodeURIComponent for main process
    const destination = global.decodeURIComponent(remoteUrl);
...
/**
 * Factory function to create CurseAddonProvider instance
 */
export function createCurseAddonProvider(
  cachingService: CachingService,
  networkService: NetworkService,
  tocService: TocService
): CurseAddonProvider {
  return new CurseAddonProvider(cachingService, networkService, tocService);
}
