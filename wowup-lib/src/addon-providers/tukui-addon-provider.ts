  ...
  private getPatchForInstall(addon: TukUiAddon, installation: WowInstallation): string {
    ...
    switch (installation.clientType) {
      ...
      case WowClientType.Anniversary:
        return addon.patch.find((p) => p.startsWith(tbcToken)) ?? '';
      default:
        return (
          addon.patch.find(
            (p) => !p.startsWith(classicToken) && !p.startsWith(tbcToken) && !p.startsWith(wrathToken) && !p.startsWith(cataToken) && !p.startsWith(mistsToken),
          ) ?? ''
        );
    }
  }
